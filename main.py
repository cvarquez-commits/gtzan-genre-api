"""
main.py — GTZAN YOLOv8 genre classifier API (Render-optimized build)

Same behavior as the original Hugging Face version, but with matplotlib
removed as a dependency to reduce memory footprint for Render's free
512MB-RAM tier. The magma colormap is baked in as a 256-entry lookup
table (generated once from matplotlib, not imported at runtime) instead
of importing matplotlib.cm live.
"""
import os
import tempfile
...
# 256-entry magma colormap, baked in from matplotlib.cm.magma so the
# deployed service never has to import matplotlib itself.
MAGMA_LUT = np.array([
...
def melspec_image(y: np.ndarray, sr: int, img_size: int = IMG_SIZE, n_mels: int = 128) -> Image.Image:
    """Convert an audio waveform into a colored mel-spectrogram image (matches training pipeline)."""
    mel = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=n_mels)
    mel_db = librosa.power_to_db(mel, ref=np.max)
    lo, hi = mel_db.min(), mel_db.max()
    mel_norm = (mel_db - lo) / (hi - lo + 1e-6)
    idx = np.clip((mel_norm * 255).astype(np.uint8), 0, 255)
    colored = MAGMA_LUT[idx]
...
@app.get("/")
def root():
    return {"status": "ok", "message": "GTZAN YOLOv8n-cls genre classifier is running"}


@app.post("/predict")
async def predict(file: UploadFile = File(...)):
    if not file.filename.lower().endswith((".wav", ".mp3", ".m4a", ".ogg", ".flac")):
        raise HTTPException(400, "Unsupported file type. Use WAV, MP3, M4A, OGG, or FLAC.")
    ...
    for i in range(n_segments):
        ...
        if len(seg) < seg_len * MIN_SEGMENT_FRACTION:
            continue
        ...
        segment_results.append({
            "segment": i,
            "predicted_genre": top_name,
            "confidence": round(top_conf, 4),
        })
        for idx2, p in enumerate(probs):
            g = names[idx2]
            genre_scores[g] = genre_scores.get(g, 0.0) + p

    if not segment_results:
        raise HTTPException(400, "Audio too short to analyze (need at least ~1.5 seconds).")

    n = len(segment_results)
    averaged = {g: s / n for g, s in genre_scores.items()}
    final_genre = max(averaged, key=averaged.get)
    final_confidence = averaged[final_genre]

    return {
        "predicted_genre": final_genre,
        "confidence": round(final_confidence, 4),
        "num_segments_analyzed": n,
        "segment_predictions": segment_results,
        "genre_probabilities": {
            g: round(v, 4) for g, v in sorted(averaged.items(), key=lambda x: -x[1])
        },
    }
